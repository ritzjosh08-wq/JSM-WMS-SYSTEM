...
export default function Outward() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const data = await fetchOutwardForCodes(codes);
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter(e =>
      [e.outwardNumber, e.truckNumber, e.transporter, e.destination]
        .filter(Boolean).some(v => String(v).toLowerCase().includes(t)));
  }, [entries, q]);

  if (loading) return <Spinner label="Loading outward entries…" />;
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle={`${filtered.length} dispatch${filtered.length === 1 ? '' : 'es'}`}
        right={
          <input className="toolbar-input" value={q} onChange={e => setQ(e.target.value)} placeholder="Search outward #, truck, destination…"
        ...
        {filtered.length ? (
          <div className="table-scroll">
            <table style={{ width: '100%' }}>
              <thead><tr>
                <th style={thStyle}>Outward #</th><th style={thStyle}>Dispatch Date</th><th style={thStyle}>Truck</th>
                <th style={thStyle}>Transporter</th><th style={thStyle}>Destination</th>
                ...
                {filtered.map(e => (
                  <Fragment key={e.id}>
                    <tr onClick={() => setOpen(open === e.id ? null : e.id)} style={{ cursor: 'pointer' }}>
                      <td style={{ ...tdStyle, fontWeight: 600, color: C.blue }}>{e.outwardNumber}</td>
                      <td style={tdStyle}>{fmtDate(e.dispatchDate || e.createdAt)}</td>
                        ...
                        <td colSpan={7} style={{ padding: 0, background: '#f8fafc' }}>
                          <table style={{ width: '100%' }}>
                            <thead><tr>
                              ...
                              <th style={{ ...thStyle, background: '#f1f5f9', textAlign: 'right' }}>Required</th>
                              <th style={{ ...thStyle, background: '#f1f5f9', textAlign: 'right' }}>Picked</th>
                            </tr></thead>
                            <tbody>
                              {e.lineItems.map((li, i) => (
                                <tr key={i}>
                                  ...
                                  <td style={{ ...tdStyle, textAlign: 'right' }}>{li.requiredQty?.toLocaleString()}</td>
                                  <td style={{ ...tdStyle, textAlign: 'right' }}>{li.pickedQty?.toLocaleString()}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
          ...
          <EmptyState message={entries.length ? 'No matches for your search.' : 'No outward entries yet.'} />
        )}
      </Card>
