...
export default function InventoryClient() {
  ...
  const binAllocation = useMemo(() => {
    const map = new Map<string, { pallets: number; kg: number; items: EnrichedItem[] }>();
    activeItems.forEach(i => {
      const bin = i.binLocation !== "—" ? i.binLocation : "Unassigned";
      if (!map.has(bin)) map.set(bin, { pallets: 0, kg: 0, items: [] });
      const cur = map.get(bin)!;
  ...
  const filtered = useMemo(() => {
    let list = enriched.filter(i => i.quantity > 0);
    ...
    return list;
  }, [enriched, view, filterWh, searchTerm, sortField, sortDir]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: "12px" }}>
        {[
          { label: "Active SKUs",    value: activeItems.length,        sub: `${enriched.length - activeItems.length} depleted`, icon: <Package size={16} />,        color: "#2563eb", bg: "#eff6ff", border: "#bfdbfe" },
          { label: "RM – Pallets",   value: totalRmPallets.toFixed(0), sub: `${rmItems.length} RM batches`,                     icon: <Layers size={16} />,          color: "#059669", bg: "#ecfdf5", border: "#a7f3d0" },
          { label: "RM – Net Wt",    value: `${totalRmKg.toFixed(0)} kg`, sub: "Total RM weight",                               icon: <Weight size={16} />,          color: "#0369a1", bg: "#e0f2fe", border: "#7dd3fc" },
          { label: "FG – Pallets",   value: totalFgPallets.toFixed(0), sub: `${fgItems.length} FG batches`,                     icon: <ArrowDownToLine size={16} />, color: "#7c3aed", bg: "#f5f3ff", border: "#ddd6fe" },
          { label: "FG – Net Wt",    value: `${totalFgKg.toFixed(0)} kg`, sub: "Total FG weight",                               icon: <Weight size={16} />,          color: "#d97706", bg: "#fffbeb", border: "#fde68a" },
        ].map(c => (
          <div key={c.label} style={{ background: c.bg, border: `1px solid ${c.border}`, borderRadius: "12px", padding: "16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "6px", color: c.color }}>
              {c.icon}
              <span style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>{c.label}</span>
            </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "14px 18px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
              <thead>
                <tr>
                  {["BIN Location", "Material Code", "Description", "Type", "Category", "Pallets", "Net Wt (kg)", "Qty (Nos)"].map(h => (
                    <th key={h} style={{ padding: "7px 10px", textAlign: "left", fontSize: "10px", fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: "0.07em", borderBottom: "1.5px solid #e2e8f0", background: "#f8fafc", whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {binAllocation.map(([bin, { pallets, kg, items }], gi) => (
                  <React.Fragment key={bin}>
                    {/* Bin header row */}
                    <tr style={{ background: "#f0f9ff" }}>
                      <td colSpan={8} style={{ padding: "5px 10px", fontWeight: 800, fontSize: "11px", color: "#0369a1", borderBottom: "1px solid #e2e8f0", borderTop: gi > 0 ? "2px solid #e2e8f0" : "none" }}>
                        <span style={{ background: "#0369a1", color: "#fff", padding: "1px 8px", borderRadius: "4px", marginRight: "8px", fontSize: "10px" }}>{bin}</span>
                        <span style={{ color: "#64748b", fontWeight: 600, fontSize: "10px" }}>{pallets.toFixed(0)} pallets · {kg.toFixed(0)} kg · {items.length} item{items.length !== 1 ? "s" : ""}</span>
                      </td>
                    </tr>
                    {/* Material rows */}
                    {items.map((item, ii) => (
                      <tr key={item.id} style={{ background: ii % 2 === 0 ? "#fff" : "#fafafa" }}>
                        <td style={{ padding: "6px 10px", color: "#cbd5e1", fontSize: "11px" }}>↳</td>
                        <td style={{ padding: "6px 10px", fontFamily: "monospace", fontWeight: 700, color: "#1e40af", fontSize: "11px" }}>{item.material?.code || "—"}</td>
                        <td style={{ padding: "6px 10px", color: "#374151", maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.material?.description || "—"}</td>
                        <td style={{ padding: "6px 10px", color: "#0891b2", fontWeight: 600, fontSize: "11px" }}>{item.materialType || "—"}</td>
                        ...
                        <td style={{ padding: "6px 10px", textAlign: "right", fontWeight: 700, color: "#7c3aed" }}>{item.displayQtyPallet > 0 ? item.displayQtyPallet.toFixed(0) : "—"}</td>
                        <td style={{ padding: "6px 10px", textAlign: "right", fontWeight: 700, color: "#059669" }}>{item.displayQtyKg > 0 ? item.displayQtyKg.toFixed(1) : "—"}</td>
                        <td style={{ padding: "6px 10px", textAlign: "right", color: "#374151" }}>{item.displayQtyNos.toFixed(0)}</td>
                      </tr>
                    ))}
                  </React.Fragment>
                ))}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: "900px" }}>
            <thead style={{ position: "sticky", top: 0, zIndex: 1 }}>
              <tr>
                ...
                <th style={{ ...thStyle, minWidth: "200px" }} onClick={() => toggleSort("description")}>
                ...
                <th style={thStyle}>Invoice / Batch</th>
                <th style={{ ...thStyle, textAlign: "right" }} onClick={() => toggleSort("quantity")}>
                  <span style={{ display: "flex", alignItems: "center", gap: "4px", justifyContent: "flex-end" }}>Qty (Nos) <SortIcon field="quantity" /></span>
                </th>
            ...
            <tbody>
              ...
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={11} style={{ ...tdStyle, textAlign: "center", padding: "48px", color: "#94a3b8" }}>
              ...
              {!loading && filtered.map((item, idx) => {
                ...
                return (
                  <tr
                    ...
                    style={{
                      background: isZero ? "#fafafa" : idx % 2 === 0 ? "#fff" : "#fafafa",
                      opacity: isZero ? 0.55 : 1,
                      transition: "background 0.1s",
                      cursor: "pointer",
                    }}
                    onMouseEnter={e => ((e.currentTarget as HTMLTableRowElement).style.background = "#eff6ff")}
                    onMouseLeave={e => ((e.currentTarget as HTMLTableRowElement).style.background = isZero ? "#fafafa" : idx % 2 === 0 ? "#fff" : "#fafafa")}
                  >
                    <td style={tdStyle}>
                    ...
                    <td style={{ ...tdStyle, maxWidth: "240px" }}>
                      <div style={{ fontWeight: 600, color: "#0f172a", fontSize: "12px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    ...
                    <td style={{ ...tdStyle, textAlign: "right" }}>
                      <QtyEditCell item={item} onSaved={load} />
                    </td>
                    <td style={{ ...tdStyle, textAlign: "right", color: item.displayQtyPallet > 0 ? "#7c3aed" : "#cbd5e1", fontWeight: 600 }}>
                      {item.displayQtyPallet > 0 ? `${item.displayQtyPallet.toFixed(0)} plt` : "—"}
                    </td>
                    <td style={{ ...tdStyle, textAlign: "right", color: item.displayQtyKg > 0 ? "#059669" : "#cbd5e1", fontWeight: 600 }}>
                      {item.displayQtyKg > 0 ? `${item.displayQtyKg.toFixed(2)} kg` : "—"}
                    </td>
