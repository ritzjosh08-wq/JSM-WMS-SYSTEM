...
catch { /* xlsx not installed yet */ }
let fflate = null;
try {
    fflate = require('fflate');
}
catch { /* fflate not installed yet — fast path below just won't run */ }
const DATE_TAB = /^(\d{1,2})[-/.](\d{1,2})[-/.](\d{4})$/;
function tabDateValue(name) {
...
function chooseSheetName(sheetNames, fileName) {
    ...
    if (dateTabs.length) {
        ...
        const matchedFromFilename = filenameDateValue !== null
            ...
            : undefined;
        chosen = matchedFromFilename || dateTabs.reduce((latest, n) => tabDateValue(n) > tabDateValue(latest) ? n : latest, dateTabs[0]);
    }
    return chosen;
}
...
function tryFastExtractSheet(buf, fileName) {
    if (!fflate)
        return null;
    try {
        ...
        const meta = fflate.unzipSync(zipBytes, { filter: (f) => metaWanted.has(f.name) });
        ...
        const parts = fflate.unzipSync(zipBytes, { filter: (f) => partsWanted.has(f.name) });
        if (!parts[sheetPath])
            return null;
        ...
        if (hasStrings)
            zipInput['xl/sharedStrings.xml'] = parts['xl/sharedStrings.xml'];
        if (hasStyles)
            zipInput['xl/styles.xml'] = parts['xl/styles.xml'];
...
function convertCell(val, headerName) {
    ...
    if (typeof val === 'number') {
        if (isDateCol && val > 1000)
            return excelSerialToDate(val);
        if (isTimeCol && val >= 0 && val < 1)
...
function run() {
    if (!XLSX) {
        worker_threads_1.parentPort.postMessage({ success: false, error: 'xlsx package not installed on backend. Run: npm install xlsx in the backend folder.' });
        return;
    }
    try {
        const { fileBase64, fileName } = worker_threads_1.workerData;
        if (!fileBase64) {
            worker_threads_1.parentPort.postMessage({ success: false, error: 'fileBase64 is required' });
            return;
        }
        ...
        if (!rawData.length) {
            worker_threads_1.parentPort.postMessage({ success: true, headers: [], rows: [] });
            return;
        }
        ...
        for (let i = 0; i < Math.min(5, rawData.length); i++) {
            const filled = rawData[i].filter((c) => String(c).trim()).length;
            if (filled > maxFilled) {
                maxFilled = filled;
                headerRowIdx = i;
            }
        }
        const rawHeaders = rawData[headerRowIdx].map((h) => String(h).trim());
        ...
        const headerNameCounts = new Map();
